...
public class Data {
  ...
  /**
   *
   * @param numItems the number of items that the returned list should content: the given supplier will
   *                 be invoked as many times as the value specified by this parameter.
   * @param itemSupplier a {@link Supplier}, used at each iteration performed within this method: supplier is
   *                     expected to create a new object at each iteration.
   * @return a new {@link List} holding the items that were created using the given supplier.
   */
  public static <T> List<T> list(int numItems, Supplier<T> itemSupplier) {
    List<T> toReturn = new ArrayList<>(numItems);
    for (int i = 0; i < numItems; i++) {
      toReturn.add(itemSupplier.get());
    }
    return toReturn;
  }
